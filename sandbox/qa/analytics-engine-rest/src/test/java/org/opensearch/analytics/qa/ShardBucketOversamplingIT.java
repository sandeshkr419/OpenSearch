...
public class ShardBucketOversamplingIT extends AnalyticsRestTestCase {
    ...
    private void ensureProvisioned() throws Exception {
        if (!provisioned) {
            ...
            // Update setting on the provisioned index
            Request req = new Request("PUT", "/" + INDEX + "/_settings");
            req.setJsonEntity("{\"index.analytics.shard_bucket_oversampling_factor\": 2.0}");
    ...
    public void testFactorZero_queryWorks() throws Exception {
        ...
        Request req = new Request("PUT", "/" + INDEX + "/_settings");
        req.setJsonEntity("{\"index.analytics.shard_bucket_oversampling_factor\": 0.0}");
        client().performRequest(req);
